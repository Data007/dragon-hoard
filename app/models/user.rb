...
class User
  ...
  accepts_nested_attributes_for :addresses, :phones, :fingers, :alliances
  ...
  # Scopes
  scope :workers,     where(:role.in => %w(employee manager admin owner administrator))
  scope :customers,   where(:role.in => %w(customer guest))
  ...
  class << self
    ...
    def find_from_query query
      ...
      nil_users      = users.reject {|user| user.last_name}
      sortable_users = users.select {|user| user.last_name}
      sortable_users = sortable_users.sort {|a,b| a.last_name <=> b.last_name}
      return sortable_users + nil_users
    end
  end
  ##
